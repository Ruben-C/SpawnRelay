...
- `internal/server` – tunnel manager (`tunnel.go`), client self-update push/download (`update.go`), HTTPS API (`api.go`, `auth.go`), embedded UI (`web/`), embedded client installers (`scripts/`)
